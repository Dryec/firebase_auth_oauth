...
public class SwiftFirebaseAppleAuthPlugin: UIViewController, FlutterPlugin, ASAuthorizationControllerDelegate {
    ...
    private func oAuthSignIn(arguments: [String: Any], result: @escaping FlutterResult) {
    ...
    private func signInWithApple(arguments: [String: Any], result: @escaping FlutterResult) {
        if #available(iOS 13, *) {
            self.result = result
    ...
    func startSignInWithAppleFlow() {
        do {
            ...
            if let data = (arguments!["scopes"] as? String)!.data(using: .utf8) {
                if let jsonArray = try JSONSerialization.jsonObject(with: data, options : .allowFragments) as? [String]
                {
